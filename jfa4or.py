...
def benchmark_jfa_2d_with_cucim(shape, seed_positions, max_iter=100):
    # Initialize seed grids for CPU and GPU
    grid_cpu = initialize_seed_grid(shape, seed_positions, device_cpu)
    grid_gpu = initialize_seed_grid(shape, seed_positions, device_gpu)
...
# Define the grid shape and seed positions
grid_shape = (9, 9)
seed_positions = [(4, 4)]

# Initialize the grid with seeds
device = torch.device('cpu')  # Use 'cuda' if GPU is available
seed_grid = initialize_seed_grid(grid_shape, seed_positions, device)

# Perform JFA propagation with debugging enabled
output_grid = jump_flood_2D(seed_grid, max_iter=2, debug=True)

# Print the result
print("\nFinal Output Grid After Propagation:")
print(output_grid)
