...
def jump_flood_3D(grid, max_iter=5, device=device_cpu, debug=False):
    ...
    while jump > 0:
        ...
        # Prepare tensors for all shifts
        rolled_grids = []
        ...
        # Compute Euclidean distances for the shifts
        ...
        # Add distances to rolled grids
        updated_grids = rolled_grids + distances

        # Take the minimum across all shifts
        min_grid, _ = torch.min(updated_grids, dim=0)  # Shape: (depth, height, width)
        grid = torch.min(grid, min_grid)  # Update the grid with the minimum distances
    ...
    # Iterative refinement to reduce errors
