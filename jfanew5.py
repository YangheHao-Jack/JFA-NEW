...
def jump_flood_3D(grid, max_iter=5, device=device_cpu, debug=False):
    ...
    # JFA propagation with parallel updates
    while jump > 0:
        shifts = [
            (dz, dy, dx)
        ...
        rolled_grids = torch.stack(rolled_grids, dim=0)  # Shape: (num_shifts, depth, height, width)

        distances = torch.tensor(
            [torch.sqrt(torch.tensor(dz**2 + dy**2 + dx**2, dtype=torch.float32, device=device))
             for dz, dy, dx in shifts],
            device=device
        ).view(-1, 1, 1, 1)  # Shape: (num_shifts, 1, 1, 1)
        ...
        if debug:
            print(f"Jump size: {jump}, grid updated.")

        jump //= 2

    # Iterative refinement without convolution
    refinement_offsets = [
        (dz, dy, dx)
    ...
    # Precompute Euclidean distances for refinement
    distances = torch.tensor(
        [torch.sqrt(torch.tensor(dz**2 + dy**2 + dx**2, dtype=torch.float32, device=device))
         for dz, dy, dx in refinement_offsets],
        device=device
    ).view(-1, 1, 1, 1)  # Shape: (num_offsets, 1, 1, 1)
    ...
    for _ in range(refinement_passes):
        ...
        rolled_grids = torch.stack(rolled_grids, dim=0)  # Shape: (num_offsets, depth, height, width)
        updated_grids = rolled_grids + distances
        min_grid, _ = torch.min(updated_grids, dim=0)  # Minimum over all neighbors
        grid = torch.min(grid, min_grid)  # Update with the minimum distances

        if debug:
            print(f"Refinement pass completed.")
